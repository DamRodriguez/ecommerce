    ...
    >
      ...
      <div className="absolute bottom-0 left-0 w-full border-t border-outline bg-surface-bright p-md custom-transition-all group-hover:-translate-y-sm">
        <div className="flex items-center justify-between gap-2">
          <span className="font-accent text-xs font-medium group-hover:font-bold tracking-[0.1em] uppercase text-on-surface custom-transition-all group-hover:tracking-[0.16em] line-clamp-1">
            {data.category}
          </span>

          <span className="font-accent text-xs font-medium tracking-[0.1em] uppercase text-secondary-text line-clamp-1 group-hover:text-on-surface custom-transition-all">
            {formatTwoDigits(data.quantity)} artículos
          </span>
        </div>

        <div className="mt-0 group-hover:mt-sm h-px w-full origin-left scale-x-0 bg-on-surface custom-transition-all group-hover:scale-x-100" />
      </div>
    </Link>
  );
}
